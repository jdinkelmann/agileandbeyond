...
@implementation agileandbeyondObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_agileandbeyond);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any agileandbeyond patches in JS before a context is loaded... nice for 
...
{
	// create agileandbeyond global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_agileandbeyond = [[agileandbeyondObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_agileandbeyond];
	
	NSString *_agileandbeyondNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _agileandbeyondNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_agileandbeyond valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.agileandbeyond.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
